...
$sql = 'SELECT ru_id, ru_software, ru_project, ru_file_name, ru_datetime FROM run WHERE ru_id ='.$ru_id;
...
if($ruRow !== false)
{
  ...
  if(file_exists($project_path) && !is_dir($project_path)) {
    ...
    $project_html = prjGetHtml($db, $ruRow, $diRow, $auguments);
  ...
  */
  ...
  $smarty->display(TEMPLATE.'/project.html');
}
